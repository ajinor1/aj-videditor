...
const zSlider = document.getElementById('zPos');
...
function drawPreview() {
    ...
    for (const clip of visibleClips) {
        ...
        if (transformedClip.type === 'text') {
            const lines = ((_a = transformedClip.text) === null || _a === void 0 ? void 0 : _a.split('\n')) || [''];
...
function applyCameraTransform(clip, cameras) {
    ...
    for (const camera of cameras) {
        transformedClip = Object.assign(Object.assign({}, transformedClip), { x: transformedClip.x - camera.x, y: transformedClip.y - camera.y, z: transformedClip.z - camera.z });
    }
...
function getClipAtPosition(cx, cy) {
    ...
    for (let i = visibleClips.length - 1; i >= 0; i--) {
        ...
        let height = 60;
        if (clip.type === 'text') {
            const lines = ((_a = clip.text) === null || _a === void 0 ? void 0 : _a.split('\n')) || [''];
...
function setupPreviewDrag() {
    ...
    const onPointerMove = (e) => {
        ...
        const canvasX = pos.x;
        const canvasY = pos.y;
        if (!isDraggingPreview && (Math.abs(canvasX - pointerStartX) < 3 && Math.abs(canvasY - pointerStartY) < 3))
            return;
...
canvas.addEventListener('wheel', (e) => {
    ...
    const textClipsAtPos = visibleClips.filter(c => {
        ...
        const lines = ((_a = c.text) === null || _a === void 0 ? void 0 : _a.split('\n')) || [''];
...
function drawTimeline() {
    ...
    if (addLayerBtn) {
        addLayerBtn.addEventListener('click', () => {
            ...
            if (addLayerInputContainer) {
                ...
                addLayerCountInput === null || addLayerCountInput === void 0 ? void 0 : addLayerCountInput.focus();
                addLayerCountInput === null || addLayerCountInput === void 0 ? void 0 : addLayerCountInput.select();
            }
        });
    }
    if (confirmAddLayerBtn) {
        confirmAddLayerBtn.addEventListener('click', () => {
            const val = parseInt((addLayerCountInput === null || addLayerCountInput === void 0 ? void 0 : addLayerCountInput.value) || '1', 10);
            if (!isNaN(val) && val > 0) {
    ...
    if (addLayerCountInput) {
        addLayerCountInput.addEventListener('keydown', (e) => {
            if (e.key === 'Enter') {
                e.preventDefault();
                confirmAddLayerBtn === null || confirmAddLayerBtn === void 0 ? void 0 : confirmAddLayerBtn.click();
            }
            if (e.key === 'Escape') {
                cancelAddLayerBtn === null || cancelAddLayerBtn === void 0 ? void 0 : cancelAddLayerBtn.click();
            }
        });
    }
...
function startResizeClip(e, clipId, edge) {
    if (isDraggingClip || isResizingClip)
        return;
...
function onResizeMove(e) {
    if (!isResizingClip || !resizeClipId || !resizeEdge)
        return;
    ...
    if (resizeEdge === 'left') {
        ...
        if (CONFIG.preventOverlap && isOverlapping(clip, clip.id)) {
            ...
            for (let i = 0; i < 100; i++) {
                ...
                if (testStartFrame < 0 || testStartFrame > endFrame - 1)
                    break;
...
function onClipDragMove(e) {
    ...
    if (newLayerId !== oldLayerId) {
        ...
        if (CONFIG.preventOverlap && isOverlapping(clip, clip.id)) {
            // レイヤー変更で重なる場合 → 空いてるレイヤーを探す
            const availableLayer = findAvailableLayer(currentStartFrame, clip.duration);
            if (availableLayer !== null) {
                clip.layerId = availableLayer;
            }
            else {
                // 空いてるレイヤーがない → 元のレイヤーに戻す
                clip.layerId = oldLayerId;
            }
        }
    }
    drawTimeline();
    ...
    while (isOverlapping(clip, ignoreId) && attempts < 100) {
        ...
        if (clip.startFrame + clip.duration > TIMELINE_DURATION) {
            ...
            if (clip.startFrame < 0) {
                clip.startFrame = 0;
                clip.duration = TIMELINE_DURATION;
            }
            break;
        }
    }
...
function setupAllNumberInputs() {
    const configs = [
        ...
        // ===== Z座標 =====
        {
            ...
            updateFn: (val) => updateSliderRange(zSlider, val, SLIDER_STAGES.coord, false),
            onCommit: (val) => {
                const selected = getSelected();
                if (!selected)
                    return;
                ...
                if (!isDraggingZ)
                    updateSliderRange(zSlider, val, SLIDER_STAGES.coord, false);
                drawPreview();
            }
        },
        // ===== Rotation =====
        ...
        // ===== FontSize =====
        {
            ...
            updateFn: (val) => updateSliderRangePositive(fontSizeSlider, val, SLIDER_STAGES.fontSize, false),
            onCommit: (val) => {
                const selected = getSelected();
                if (!selected)
                    return;
                ...
                if (!isDraggingFontSize)
                    updateSliderRangePositive(fontSizeSlider, val, SLIDER_STAGES.fontSize, false);
                drawPreview();
            }
        },
        // ===== Stroke W =====
        {
            ...
            updateFn: (val) => updateSliderRangePositive(strokeWidthSlider, val, SLIDER_STAGES.stroke, false),
            onCommit: (val) => {
                const selected = getSelected();
                ...
                if (!isDraggingStroke)
                    updateSliderRangePositive(strokeWidthSlider, val, SLIDER_STAGES.stroke, false);
                drawPreview();
            }
        },
        // ===== Width =====
        {
            ...
            updateFn: (val) => updateSliderRangePositive(shapeWidthSlider, val, SLIDER_STAGES.size, false),
            onCommit: (val) => {
                const selected = getSelected();
                if (!selected || selected.type !== 'shape')
                    return;
                ...
                if (!isDraggingWidth)
                    updateSliderRangePositive(shapeWidthSlider, val, SLIDER_STAGES.size, false);
                drawPreview();
            }
        },
        // ===== Height =====
        {
            ...
            updateFn: (val) => updateSliderRangePositive(shapeHeightSlider, val, SLIDER_STAGES.size, false),
            onCommit: (val) => {
                const selected = getSelected();
                if (!selected || selected.type !== 'shape')
                    return;
                ...
                if (!isDraggingHeight)
                    updateSliderRangePositive(shapeHeightSlider, val, SLIDER_STAGES.size, false);
                drawPreview();
            }
        },
        // ===== Camera Range =====
...
setupSliderDrag(zSlider, () => { isDraggingZ = true; }, () => {
    isDraggingZ = false;
    const selected = getSelected();
    if (selected) {
