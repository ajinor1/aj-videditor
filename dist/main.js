...
function setupPreviewDrag() {
  ...
  const onPointerMove = e => {
    ...
    if (
      !isDraggingPreview &&
      Math.abs(canvasX - pointerStartX) < 3 &&
        Math.abs(canvasY - pointerStartY) < 3
    )
      return
...
function drawTimeline() {
  ...
  for (let layerId = 1; layerId <= currentLayerCount; layerId++) {
    ...
    for (const clip of layerClips) {
      ...
      html += `<div class="timeline-clip ${isSelected ? "selected" : ""} ${
        isDragging ? "dragging" : ""
      }" 
                          data-clip-id="${clip.id}"
                          style="left:${left}px; width:${Math.max(
        width,
        4
      )}px; background:${color}; opacity:${opacity};">
  ...
  const containerHeight =
    timelineContainer.clientHeight || Math.min(totalHeight + 8 + 32, 500)
  ...
  const addLayerInputContainer = document.getElementById(
    "addLayerInputContainer"
  )
  ...
  if (timelineControls && !document.getElementById("zoomControls")) {
    ...
    zoomLevelDisplay.style.cssText =
      "font-size:12px; color:var(--text-secondary); min-width:44px; text-align:center;"
    updateZoomDisplay()
...
function onClipDragMove(e) {
  ...
  if (CONFIG.preventOverlap && isOverlapping(clip, clip.id)) {
    ...
    while (attempts < 100 && !found) {
      ...
      if (
        testFrame < 0 ||
        testFrame > TIMELINE_DURATION * CONFIG.fps - clip.duration
      ) {
        break
      }
    }
  ...
  if (newLayerId !== oldLayerId) {
    ...
    if (CONFIG.preventOverlap && isOverlapping(clip, clip.id)) {
      ...
      const availableLayer = findAvailableLayer(
        currentStartFrame,
        clip.duration
      )
...
function getPixelsPerSecond(containerWidth) {
  const usableWidth =
    containerWidth - TIMELINE_PADDING_LEFT - TIMELINE_PADDING_RIGHT
  const visibleDuration = getVisibleDuration()
...
function getTotalTimelineWidth(containerWidth) {
  ...
  const visibleDuration = getVisibleDuration()
  return (
    (TIMELINE_DURATION / visibleDuration) * usableWidth +
...
function getTickInterval() {
  ...
  if (visibleDuration <= 5) return 1.0
  if (visibleDuration <= 10) return 2.0
  if (visibleDuration <= 20) return 5.0
  return 10.0
}
...
function zoomTimeline(factor) {
  ...
  const headPixel =
    (currentFrame / CONFIG.fps) * oldPixelsPerSecond + TIMELINE_PADDING_LEFT
  ...
  const newHeadPixel =
    (currentFrame / CONFIG.fps) * newPixelsPerSecond + TIMELINE_PADDING_LEFT
...
function syncUI() {
  ...
  if (selected && hasClips) {
    ...
    updateSliderRangePositive(
      fontSizeSlider,
      selected.fontSize || 50,
      SLIDER_STAGES.fontSize,
      isDraggingFontSize
    )
...
function addClip(type) {
  ...
  if (layerId === null) {
    alert(
      "これ以上クリップを追加できません。レイヤー数を増やすか、既存のクリップを移動してください。"
    )
    return
  }
...
function onHorizontalResize(e) {
  ...
  const parentWidth =
    canvasWrapper.parentElement.getBoundingClientRect().width - 6
...
function onVerticalResize(e) {
  ...
  const topSectionHeight = document
  ...
  const newHeight = Math.min(
    Math.max(resizeStartHeight + delta, MIN_TIMELINE_HEIGHT),
    totalHeight * 0.6
  )
...
window.__editor = {
  ...
  setFrame: frame => {
    ...
    drawTimeline()
    console.log(
      `Frame set to ${currentFrame} (${(currentFrame / CONFIG.fps).toFixed(
        2
      )}s)`
    )
  },
